...
const Signup = () => {
  ...
  const handleGoogleSignup = () => {
    // Mock Google signup - create a demo user
    const mockGoogleUser = {
      email: 'google.newuser@gmail.com',
      name: 'New Google User',
      role: 'user',
      bio: 'New professional signed up via Google'
    };
    
    const mockToken = 'mock-google-signup-token-' + Date.now();
    localStorage.setItem('token', mockToken);
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <form className="mt-8 space-y-6 bg-white p-8 rounded-xl shadow-lg" onSubmit={handleSubmit}>
          ...
          <div className="bg-green-50 border border-green-200 text-green-800 px-4 py-3 rounded-lg">
            <p className="text-sm">
              <strong>Demo Mode:</strong> Create an account with any email and password (min. 6 characters). 
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder={formData.role === 'recruiter' ? 'Enter company name' : 'Enter your full name'}
                value={formData.name}
                onChange={handleChange}
              />
            </div>
            ...
            <div>
              <label htmlFor="bio" className="block text-sm font-medium text-gray-700">
                {formData.role === 'recruiter' ? 'About Company' : 'Professional Bio'}
              </label>
              <textarea
                id="bio"
                name="bio"
                rows="3"
                className="mt-1 appearance-none relative block w-full px-3 py-3 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-lg focus:outline-none focus:ring-blue-500 focus:border-blue-500 focus:z-10"
                placeholder={formData.role === 'recruiter' ? 'Tell us about your company...' : 'Tell us about your professional background...'}
                value={formData.bio}
                onChange={handleChange}
              />
            </div>
